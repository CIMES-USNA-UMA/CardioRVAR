...
```
As there is a breakpoint, the data is not stationary. We will separate the data into two stationary subsets, Pre and Post. We will also detrend this data:

```{r Check stationarity again, eval = TRUE}
Pre <- Data[1:400,]
Post <- Data[500:928,]

Pre[,"IBI"] <- DetrendByCutoff(Pre[,"IBI"])
Pre[,"SBP"] <- DetrendByCutoff(Pre[,"SBP"])
Post[,"IBI"] <- DetrendByCutoff(Post[,"IBI"])
Post[,"SBP"] <- DetrendByCutoff(Post[,"SBP"])
